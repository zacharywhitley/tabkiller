...
// in query output. Time origin = 0 = "midnight on the spike day". All times
// in ms since that origin.
//
...
// ---- Node builders (no defaults magic; explicit) ----
...
function visit(
  ...
  return { id, type: 'Visit', recorded_at: at_time, at_time, ended_at, focus_intervals, transition };
}
...
function session(id: string, started_at: number, ended_at: number | null, detected_by: SessionNode['detected_by']): SessionNode {
...
function searchQuery(id: string, engine: SearchQueryNode['engine'], query_text: string, results_url: string, at_time: number): SearchQueryNode {
...
// Formatting helper for readable query output
